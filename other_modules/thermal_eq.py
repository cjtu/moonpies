...
def main():
    ...
    #coldtrap_craters = ['Haworth', 'Shoemaker', 'Faustini', 'Shackleton', 'Amundsen', 'Sverdrup', 'Cabeus B', 'de Gerlache', "Idel'son L", 'Wiechert J']
    coldtrap_indicies = [0,1,2,3,5,6,8,14,21,22]
    coldtrap_temps = [45.,50.,50.,100.,90.,70.,70.,60.,70.,70.]

    Teq = np.zeros((len(df), len(df)))
    Teq_ct = np.zeros((len(df), len(coldtrap_temps)))
    ...
    for i in range(0,24):
        for c, j in enumerate(coldtrap_indicies):
            if i != j: #can't dump ejecta into yourself
                ...
                print(df["cname"][i], "into", df["cname"][j], "at", ej_distances[i,j], "m, with vf =", vf, "and temp =", 250.+DT)
...
def read_crater_list(columns, rp=1737e3):
    ...
    df = pd.read_csv("code/crater_list.csv", names=columns, header=0)
...
def slow(alpha, T_clast_i, T_surf, vf):
    ...
    T (array): evolved temperature profile of the mixed material [K]
    """
    T = np.full((100, 100000), T_clast_i)#T_surf) #1,000 steps = 10 cm total, 100 micron steps
    T = vol_frac(T, T_surf, vf)
    ...
    plt.savefig("Evolving_Temps.png")    
    plt.show()
    return T  

Teq, Teq_ct = main()
np.savetxt("Teqs.csv", Teq, delimited=',') #all craters into all craters, 24x24 grid
np.savetxt("bal_sed_cold_traps.csv", Teq_ct, delimited=',') #all craters into all cold traps, 24x10 grid